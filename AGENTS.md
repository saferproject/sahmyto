...
Use Bun 1.4.2 exclusively for dependency management and script execution (the version is pinned in `package.json` and the Docker image). Commit `bun.lock` whenever dependencies change. Keep package manager configuration in `package.json` and `bunfig.toml`, and use only the Bun lockfile.
